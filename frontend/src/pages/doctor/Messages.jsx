...
export default function DoctorMessages() {
  ...
  const [messages, setMessages] = useState([
    ...
    { id: 2, sender: 'patient', text: 'Good morning Dr. Varma! It feels much better, minimal stiffness and no pain when doing the ankle pumps.', time: '09:22 AM', read: true },
  ...
  const contacts = [
    { id: 1, name: 'Rahul Kumar', uhid: 'P-102', surgery: 'Total Knee Replacement', lastMsg: 'Keep the incision area dry...', time: '09:25 AM', unread: 0 },
    { id: 2, name: 'Vikram Mehta', uhid: 'P-101', surgery: 'Cardiac Bypass', lastMsg: 'Feeling feverish this morning.', time: '08:22 AM', unread: 1 },
    { id: 3, name: 'Anil Sengupta', uhid: 'P-104', surgery: 'Spinal Fusion', lastMsg: 'Swelling around lower back.', time: 'Yesterday', unread: 0 },
  ]
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Clinical Telehealth Inbox"
        description="Secure bidirectional direct communication with post-operative surgical patients"
      />
            ...
            {contacts.map((c) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-2xs text-primary font-medium">{c.surgery}</p>
        ...
        <div className="md:col-span-8 flex flex-col justify-between bg-bg/40">
          <div className="p-3.5 bg-white border-b border-border flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-bold text-text">Rahul Kumar (P-102)</h3>
                <p className="text-2xs text-text-muted">Total Knee Arthroplasty (L) · Day 14 post-op</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3">
            {messages.map((m) => {
              const isDoc = m.sender === 'doc'
              return (
                <div key={m.id} className={`flex flex-col ${isDoc ? 'items-end' : 'items-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-[10px] text-xs leading-relaxed shadow-xs ${
                    isDoc ? 'bg-primary text-white rounded-br-none' : 'bg-white text-text border border-border rounded-bl-none'
                  }`}>
                    {m.text}
                  </div>
                  <span className="text-2xs text-text-muted mt-1 flex items-center gap-1">
                    {m.time} {isDoc && <CheckCheck size={12} className="text-primary" />}
                  </span>
                </div>
              )
            })}
